    ...
    private function get_rows($table_id) {
        ...
        $rows = [
            ...
            [
                ...
                'value' => WOOT_HELPER::draw_switcher('use_load_more_button', $this->get($table_id, 'use_load_more_button', false), $table_id, 'woot_save_table_option'),
                'value_custom_field_key' => 'use_load_more_button',
                'notes' => esc_html__('Use load more button instead of pagination. If it is activated, pagination and per-page drop-down will be hidden', 'profit-products-tables-for-woocommerce')
            ],
            [
                'id' => $table_id,
        ...
        if (woot()->show_notes) {
            foreach ($rows as $k => $row) {
                if ($row['value_custom_field_key'] === 'use_load_more_button') {
                    $rows[$k] = [
                        'id' => $table_id,
                        'title' => esc_html__('Use load more button', 'profit-products-tables-for-woocommerce'),
