...
class WOOT_Predefinition {
    ...
    private function get_rows($table_id) {
        ...
        if ($WOOT_Favourites) {
            $rows[] = [
                'id' => $table_id,
                'title' => esc_html__('Favourites only', 'profit-products-tables-for-woocommerce'),
                'value' => WOOT_HELPER::draw_switcher('favourites_only', intval($this->get($table_id, 'favourites_only')), $table_id, 'woot_save_table_predefinition_field'),
                'value_custom_field_key' => 'favourites_only',
                'notes' => esc_html__('Show favourites products only', 'profit-products-tables-for-woocommerce')
            ];
        }
